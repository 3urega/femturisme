# Document 01 — Presa de requeriments

## Agent d'IA per a femturisme.cat

| Camp | Valor |
|------|-------|
| Projecte | Agent d'IA per a femturisme.cat |
| Document | Presa de requeriments |
| Versió | 0.1 |
| Data | 2026 |
| Estat | Esborrany inicial |
| Destinataris | Direcció, equip femturisme, consultora, equip tècnic |

Documents relacionats: [funcional.md](funcional.md), [tecnic.md](tecnic.md), [dominio-femturisme-ca.md](dominio-femturisme-ca.md).

## Índex

- [1. Context del projecte](#1-context-del-projecte)
- [2. Objectiu general](#2-objectiu-general)
- [3. Objectius específics](#3-objectius-específics)
- [4. Fases del projecte](#4-fases-del-projecte)
- [5. Actors del sistema](#5-actors-del-sistema)
- [6. Capacitats de l'agent](#6-capacitats-de-lagent)
- [7. Principis de funcionament](#7-principis-de-funcionament)
- [8. Filosofia de l'agent](#8-filosofia-de-lagent)
- [9. Requeriments funcionals](#9-requeriments-funcionals)
- [10. Requeriments no funcionals](#10-requeriments-no-funcionals)
- [11. Restriccions del projecte](#11-restriccions-del-projecte)
- [12. Criteris d'acceptació](#12-criteris-dacceptació)
- [13. Indicadors d'èxit (KPIs)](#13-indicadors-dèxit-kpis)
- [14. Evolució prevista](#14-evolució-prevista)
...
- Assistent general de femturisme (**mode femturisme**).
- Consulta exclusiva del catàleg MySQL (6 buscadors).
- Preguntes de recomanació turística i de coneixement **a partir del catàleg** (fitxes, articles, agenda, etc.).
- **Sense RAG** en el xat de femturisme.cat: no es consulta documentació indexada encara que existeixi al sistema.

### Fase 2

- Gestor d'entitats i gestió documental per entitat.
- Assistent d'entitats (**mode entitat**): RAG exclusiu de l'`entity_id` indicat.
- Integració portal ↔ entitat: associació opcional d'`entity_id` a fitxes del catàleg.
- **RAG condicional en mode femturisme**: només si algun element retornat per una Tool de catàleg porta `entity_id`; en cap altre cas.
- Desplegament inicial amb **pocs elements de prova** amb entitat associada; l'ampliació dependrà de l'equip comercial.

> **Decisió de producte (Jordi):** en mode femturisme, el 99,9 % de les consultes seran només catàleg MySQL. El RAG no és una font general del portal; és un complement puntual quan una fitxa concreta té entitat vinculada. Per això la integració RAG al xat públic queda a Fase 2.
...
- Consultes sobre guies o documents PDF indexats (**Fase 2**: mode entitat, o mode femturisme només si la fitxa consultada té `entity_id`).
...
| Actor | Descripció |
|-------|------------|
| **Visitant** | Usuari final que interactua amb l'agent mitjançant llenguatge natural per obtenir informació turística. |
| **Administrador de continguts** | Personal de femturisme responsable de mantenir les fonts d'informació addicionals (per exemple, guies municipals). |
| **Administrador tècnic** | Responsable del desplegament, configuració, monitorització i manteniment tècnic del sistema. |
| **Sistema femturisme.cat** | Portal web que integra l'agent i proporciona accés al catàleg de continguts. |
| **Agent d'IA** | Component responsable d'interpretar les preguntes, consultar les fonts d'informació disponibles i generar les respostes. |
...
> **Usuari:** Vull anar al Berguedà.
>
> **Després:** I on puc dormir allà?
...
| ID | Principi | Descripció |
|----|----------|------------|
| **P1** | Prioritzar les fonts pròpies | Les respostes es construiran utilitzant preferentment la informació disponible a femturisme.cat. |
| **P2** | Informació fiable | Només es retornarà informació procedent de fonts conegudes i controlades. |
| **P3** | No inventar informació | En absència d'informació suficient, l'agent ho indicarà a l'usuari. |
| **P4** | Orientació al visitant | Les respostes hauran de ser útils, naturals i orientades a ajudar el visitant. |
| **P5** | Promoció del contingut del portal | Sempre que sigui possible, l'agent facilitarà l'accés a les pàgines corresponents de femturisme.cat. |
| **P6** | Evolució futura | La primera versió treballarà exclusivament amb fonts internes. No obstant això, el sistema haurà de permetre incorporar, en futures versions, fonts externes mantenint clarament diferenciada la procedència de la informació. |
...
Quan la informació disponible sigui insuficient o no existeixin coincidències exactes, haurà d'intentar ajudar l'usuari proposant alternatives raonables. Per exemple:
...
**Exemple:** «Vull passar un cap de setmana a la Garrotxa.»
...
> **Usuari:** Què puc fer aquest cap de setmana?
>
> **Agent:** En quina comarca o població?
...
| Mode | Descripció |
|------|------------|
| **Mode femturisme** | Accés al catàleg MySQL del portal. **Fase 1:** sense RAG. **Fase 2:** RAG només si algun resultat de catàleg inclou `entity_id`. |
| **Mode entitat** | Accés exclusivament a la base de coneixement documental de l'`entity_id` indicat (RAG). |

### RF-15. Integració amb entitats

Els elements del portal podran tenir associada una `entity_id` (Fase 2).

En **mode femturisme**, l'agent **no** consultarà RAG de forma general ni per municipi/territori. Només invocarà la base de coneixement d'entitat quan:

1. una Tool de catàleg retorni un o més elements amb `entity_id` a la resposta, i
2. la pregunta de l'usuari justifiqui complementar aquella fitxa amb documentació indexada.

Si cap element de la resposta té `entity_id`, la conversa es resol exclusivament amb el catàleg MySQL.

Inicialment només s'associaran **un parell d'elements de prova**; l'escala posterior la definirà l'equip comercial segons resultats.
...
| Àrea | Requeriment |
|------|-------------|
| **Rendiment** | L'agent haurà de proporcionar respostes en un temps compatible amb una conversa interactiva. |
| **Disponibilitat** | El sistema haurà d'estar disponible durant els períodes habituals de funcionament del portal. |
| **Seguretat** | L'agent només podrà accedir a les fonts d'informació autoritzades. |
| **Escalabilitat** | L'arquitectura haurà de permetre ampliar les funcionalitats de l'agent sense redissenyar completament el sistema. |
| **Mantenibilitat** | Les diferents fonts d'informació hauran de poder evolucionar de manera independent. |
| **Traçabilitat** | Les consultes realitzades per l'agent hauran de poder ser registrades amb finalitats de diagnòstic i millora contínua. |
...
- L'assistent públic de femturisme.cat treballarà exclusivament en **mode femturisme** i **sense RAG** (només catàleg MySQL).
- La gestió d'entitats, les bases documentals i la integració RAG al xat públic formen part de la **Fase 2**.
...
| ID | Criteri |
|----|---------|
| **CA-01** | L'usuari pugui interactuar amb l'agent mitjançant llenguatge natural. |
| **CA-02** | L'agent sigui capaç d'interpretar correctament la intenció de l'usuari en la majoria de consultes habituals. |
| **CA-03** | L'agent consulti les fonts d'informació necessàries sense que l'usuari conegui el seu funcionament intern. |
| **CA-04** | L'agent combini correctament informació procedent de diverses fonts quan la situació ho requereixi. |
| **CA-05** | Les respostes incloguin enllaços cap als continguts rellevants de femturisme.cat. |
| **CA-06** | Quan no existeixin coincidències exactes, l'agent proposi alternatives útils. |
| **CA-07** | L'agent mantingui el context durant tota la conversa. |
| **CA-08** | L'agent no inventi informació. |
| **CA-09** | El sistema permeti incorporar noves fonts d'informació sense modificar el model funcional de l'agent. |
...
| Indicador | Objectiu |
|-----------|----------|
| Percentatge de consultes resoltes sense aclariments | > 70 % |
| Percentatge de consultes resoltes sense errors | > 95 % |
| Percentatge de consultes que retornen contingut rellevant | > 90 % |
| Temps mitjà de resposta | < 5 segons (objectiu inicial) |
| Percentatge de respostes amb enllaços a femturisme | Monitorització |
| Percentatge de consultes sense resposta | Monitorització |
| Percentatge de recomanacions acceptades pels usuaris | Evolució futura |
